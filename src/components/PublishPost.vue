        ...
        <div v-if="showRecommend" class="tag-recommend">
          ...
          <div class="tag-operation-bar" style="margin: 12px 0;">
            ...
            <div class="tag-search" style="display: flex;align-items: center;margin-right: 16px;">
              <el-input v-model="searchTagInput" placeholder="搜索标签" size="small"
                style="width: 200px;margin-right: 8px;" @keydown.enter.prevent="handleSearchTags" />
            ...
            <div class="tag-add" style="display: flex;align-items: center;">
              <el-input v-model="newTagInput" placeholder="输入新标签" size="small"
                style="width: 200px;margin-right: 8px;" @keydown.enter.prevent="handleAddTag" />
          ...
          <div class="recommend-list">
            <span v-for="tag in recommendTags" :key="tag.tagsId" class="recommend-tag"
...
import {
  ...
  getContentLength, removeTag, deleteLastTag, addRecommendTag, isContentChanged
} from '@/utils/publish';

// 导入API接口（含新增的标签搜索/添加接口）
import { uploadImg } from '@/api/personal';
import { getRecommendTags, addTags, searchTags, updatePost } from '@/api/post';
...
export default {
  ...
  data() {
    ...
    return {
      ...
      nodeList: ['交流', '提问', '工作', '摸鱼', '日常', '游戏', '美图', '旅游'], // 节点列表
      tagInput: '', // 手动输入标签的输入值
  ...
  mounted() {
    const processTagsList = () => {
      // 如果tagsList是对象数组，提取tagsId；否则保持原数组（ID数组）
      if (this.postForm.tagsList && Array.isArray(this.postForm.tagsList)) {
        this.postForm.tagsList = this.postForm.tagsList.map(item =>
          typeof item === 'object' ? item.tagsId : item
        );
      }
    };
    // 从本地存储加载草稿
  ...
  methods: {
    ...
    sonEdit() {
      ...
      this.$refs.postFormRef.validate(valid => {
        if (valid) {
          this.postForm.createTime = new Date().toISOString();
          this.handleEdit('post', this.postForm);
    ...
    handleEdit(type, val) {
      ...
      const updateBackParams = {
        postId: Number(val.postId),
        title: val.title,            
        node: val.node,              
        content: val.content,        
        tagsIds: val.tagsList.map(id => Number(id))  
      };
      console.log("updateBackParams的数据："+updateBackParams);
      updatePost(updateBackParams).then(res => {
